# Kincaid Wolstein LifeCarePlan Application
...
```bash
mkvirtualenv lifecareplan -p python3.11
...
Create a database named `lifecareplan`.

```
createdb lifecareplan
```
...
```bash
celery -A lifecareplan worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A lifecareplan worker -l INFO -B --pool=solo
```
